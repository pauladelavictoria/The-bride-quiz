...
const Main = (props) => {
  ...
  const playerName = props.authenticatedUser?.name || "Invitada";

  return (
    <div className="main">
      <div className="main_status-bar">
        <div className="main_status-info">
          <span>Despedida: <strong>{groupName}</strong> ({groupCode})</span>
          <span className="main_status-separator">|</span>
          <span>Jugadora: <strong>{playerName}</strong></span>
        </div>
        <button className="main_logout-btn" onClick={props.onLogout}>
          Cerrar sesión
        </button>
      </div>
